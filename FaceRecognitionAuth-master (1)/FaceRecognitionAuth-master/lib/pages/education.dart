...
class _educationState extends State<education> {
  ...
  Future SendData() async {
    ...
    final encNat = base64Encode(aes.encrypt(Uint8List.fromList(utf8.encode(nationalid.text))));
    final encAmount = base64Encode(aes.encrypt(Uint8List.fromList(utf8.encode(amount.text))));
    final encPurpose = base64Encode(aes.encrypt(Uint8List.fromList(utf8.encode(purpose.text))));
  ...
  Future SendData2(var date,var time) async {
    ...
    final encTooo = base64Encode(aes.encrypt(Uint8List.fromList(utf8.encode(dropdown))));
    final encType = base64Encode(aes.encrypt(Uint8List.fromList(utf8.encode('Education'))));
    final encAmount = base64Encode(aes.encrypt(Uint8List.fromList(utf8.encode(amount.text))));
                  ...
                  Container(
                      ...
                      child: ClipRRect(
                        child: SizedBox(height: 50, child:  ElevatedButton(
                          ...
                          onPressed: () async {
                            setState(() {
                              if(nationalid.text!=''&&amount.text!=''&&purpose.text!=''&&pass!='')
                              {
                                if(pass.text==pass2) {
