...
class _electricityState extends State<electricity> {
  ...
  String dropdown = 'Injas Electricity';
  var items = ['Injas Electricity','Canal Electricity','Middle Egypt Electricity','North Delta Electricity','North Cairo Electricity','South Delta Electricity'];
  ...
  Future SendData() async {
    ...
    var url = Uri.parse('https://inconspicuous-pairs.000webhostapp.com/electricity.php') ;


    final key = "2f7b4e8d71c4a00f2a3f4c175a8a4e6c";
    final aes = Aes(key);

    final encNat = base64Encode(aes.encrypt(Uint8List.fromList(utf8.encode(dropdown))));
    final encNumber = base64Encode(aes.encrypt(Uint8List.fromList(utf8.encode(code.text))));
    final encAmount = base64Encode(aes.encrypt(Uint8List.fromList(utf8.encode(amount.text))));


    final response = await http.post(url, body:{

      "electricitytype":encNat,
      "electricitynumber":encNumber,
      "amount":encAmount,
  ...
  Future SendData2(var date,var time) async {
    ...
    final encType = base64Encode(aes.encrypt(Uint8List.fromList(utf8.encode('Electricity'))));
    final encAmount = base64Encode(aes.encrypt(Uint8List.fromList(utf8.encode(amount.text))));
          ...
          ListView(
            ...
            children: <Widget>[
              ...
              Column(
                children: <Widget>[
                  SizedBox(height: 40),
                  Center(child: Text('Electricity'.tr,style: TextStyle(fontSize: 19,fontWeight: FontWeight.bold),)),
                  SizedBox(height: 40),
                  Row(children: [
                    Container(
                  ...
                  Row(children: [
                    Container(
                      padding: EdgeInsets.only(left: 50),
                      child: Text('Your electricity code number'.tr,style: TextStyle(fontSize: 18,fontWeight: FontWeight.bold)),
                    ),
                  ],),
                  Container(
                    width: 310,
